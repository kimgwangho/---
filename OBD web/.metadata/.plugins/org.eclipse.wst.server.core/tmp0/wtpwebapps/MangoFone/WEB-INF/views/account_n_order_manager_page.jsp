	...
	<head>
		...
		<script type="text/javascript">	
			$(document).ready(function(){				
					...
					$.ajax({
				        ...
				        success : function(response) {
				        	...
				        	for(var i =0; i < response.length; i++){
								...
								if(response[i].finish_yn == 'y') {
									finish_yn = '<td style="color:blue">완료</td>'
								} else if(response[i].finish_yn == 'n') {
									finish_yn = '<td style="color:red">대기</td>'
								} else {
									finish_yn = '<td style="color:red">' + response[i].finish_yn + '</td>'
								}
				        		...
				        		add_line = '<tr>' +
									...
									pay_yn +
									finish_yn +
									//'<td>' + (response[i].pay_yn == 'y'? "완료":"대기")+ '</td>' +
									//'<td>' + (response[i].finish_yn == 'y'? "완료":"대기")+ '</td>' +
									'<td>' + (response[i].finish_time == null?"":response[i].finish_time) + '</td>' +
									'<td> <button class="" name="btn_pay_yn" onclick="">완료</button> </td>' +
									'<td> <button class="" name="btn_finish_yn" onclick="">완료</button> </td>' +								
								'</tr>';
				...
				$(document).on('click', 'button[name=btn_finish_yn]', function() {
					var param;
					var check_btn = $(this);
					...
					var answer = confirm(name + "님의 처리가 완료 되었습니까?")
					if(!answer) {
						return;
					}
					
					$.ajax({
				        url : "<c:url value='/update_order_finish.json'/>",
				        type: 'post',
				        dataType: 'json',
				        data: param,
				        error : function() {
				        ...
				        success : function(response) {
				        	if(response == 0) {
				        		var date_time;
								var date = new Date();
								date_time = date.getFullYear() + "-" + leadingZeros((date.getMonth() +1) ,2) + "-" + leadingZeros(date.getDate(), 2) 
											+ " " + leadingZeros(date.getHours(), 2) + ":" + leadingZeros(date.getMinutes(), 2) + ":" + leadingZeros(date.getSeconds(), 2);

					        	td.eq(8).html("완료");
					        	td.eq(8).css("color", "blue")
					        	td.eq(9).html(date_time); 
				        	} // if
						} // success : function
					}); // $.ajax
				});
				
				$(document).on('click', 'button[name=btn_pay_yn]', function() {
					var param;
					var check_btn = $(this);
					var tr = check_btn.parent().parent();
					var td = tr.children();
					...
					var answer = confirm(name + "님의 결제가 완료 되었습니까?")
					if(!answer) {
						return;
					}
					
					$.ajax({
				        url : "<c:url value='/update_order_pay.json'/>",
				        type: 'post',
				        dataType: 'json',
				        data: param,
				        error : function() {
		...
		<div id="container" style="width:100%">
			...
			<div id="content" class="right_col"> 
				...
				<section class="content">
					<table class="table table-striped table-bordered table-hover no-footer dtr-inline collapsed" id="list_table" style="width:99%;">
						<thead>
							<tr role="row">
								<th>번호</th>
								<th>주문번호</th>
